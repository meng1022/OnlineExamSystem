  ...
  <div class="main">
    <a-form :form="form" class="user-layout-login" ref="formLogin" id="formLogin">
      <a-tabs
        ...
        <template v-if="!$isInfield">
        <a-tab-pane key="tab1" tab="手机号 / 账号登录">
            <a-form-item>
              <a-input
                size="large"
...
<!--                <a @click="register" class="register">-->
<!--                  注册-->
<!--                </a>-->
                <a @click="forget" class="forge-password">
                  &nbsp;&nbsp;&nbsp;&nbsp;忘记密码
                </a>
              </a-form-item>
            </a-form-item>
          ...
          <template v-else>
            <a-tab-pane key="tab1" tab="姓名 / 身份证后六位登录">
            ...
            <a-form-item>
              <a-select
                size="large"
                v-decorator="['provcode',validatorRules.provcode]"
                placeholder="请选择省份"
                @change="provChange">
            ...
            <a-form-item>
              <a-select
                ...
                placeholder="请选择姓名"
                @change="nameChange">
                <a-select-option value="">请选择姓名</a-select-option>
                <a-select-option v-for="name in nameList" :key="name">{{name}}</a-select-option>
                <a-icon slot="prefix" type="lock" :style="{ color: 'rgba(0,0,0,.25)' }"/>
              </a-select>
            </a-form-item>
            ...
            <a-form-item>
              <a-input
                v-decorator="['password',validatorRules.idcard]"
                size="large"
                type="password"
                autocomplete="false"
...
<script>
  ...
  import { getInfieldList } from '@/api/api'
    ...
    data () {
      return {
        ...
        validatorRules:{
          ...
          idcard:{rules: [{ required: true, message: '请输入身份证后六位!',validator: 'click'}]},
          provcode:{rules: [{ required: true, message: '请选择省份!',validator: 'click'}]},
          name:{rules: [{ required: true, message: '请选择姓名!',validator: 'click'}]},
        },
        verifiedCode:"",
        inputCodeContent:"",
        inputCodeNull:true,
...
//        })
    ...
    methods: {
      ...
      initData () {
        let self = this
        if (this.$isInfield) {
          getInfieldList().then(function(res){
            self.infieldList = res.result
            self.provinceData = []
            self.nameList = []
            self.infieldList.forEach(function(item){
              self.provinceData.push(item.prov)
            })
          })
        }
      },
      provChange (value) {
        let self = this
        this.infieldList.forEach(function(item){
          if (item.prov == value) {
            self.nameList = item.users
          }
        })
      },
      ...
      handleSubmit () {
        ...
        if (that.customActiveKey === 'tab1') {
          that.form.validateFields([ 'username', 'password','inputCode', 'provcode', 'name' ], { force: true }, (err, values) => {
            if (!err) {
