    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="省" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input placeholder="请输入省份" v-decorator="[ 'provcode', validatorRules.provcode]" />
        </a-form-item>
        <a-form-item label="市" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input placeholder="请输入市" v-decorator="[ 'citycode', validatorRules.citycode]" />
        </a-form-item>
  ...
  export default {
    ...
    data () {
      return {
        ...
        validatorRules:{
          ...
          proftitle:{rules:[{message: '请输入职称!'}]},
          orgname:{rules:[{message: '请输入单位!'}]},
          provcode:{rules:[{message: '请输入正确的省份!'}]},
          citycode:{rules:[{message: '请输入正确的市!'}]},
    ...
    methods: {
      ...
      edit (record) {
        ...
        that.$nextTick(() => {
          that.form.setFieldsValue(pick(this.model,'username','sex','realname','email','phone','activitiSync','citycode','orgname','proftitle','provcode'))
        });
        // 调用查询用户对应的部门信息的方法
