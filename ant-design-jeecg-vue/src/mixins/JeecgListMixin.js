...
export const JeecgListMixin = {
  ...
  created() {
    ...
    if(!this.$route.params.sessionName)
      this.queryParam.sessionName = this.$route.params.sessionName
    this.loadData();
    //初始化字典配置 在自己页面定义
  },
  methods:{
    ...
    getQueryField() {
      ...
      this.columns.forEach(function (value) {
        if(value.dataIndex!=null&&value.dataIndex!='')
          str += "," + value.dataIndex;
      });
      return str;
    },
    ...
    //为在某个试题库中重置所重写的重置方法，因为此时sessionname有值
    searchReset1(){
      this.queryParam={}
      this.queryParam.sessionName = this.$route.params.sessionName
      this.loadData(1);
    ...
    handleImportExcel(info){
      console.log("info.file:  "+info.file.toString())
